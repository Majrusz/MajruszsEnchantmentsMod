...
public class DodgeParticle extends SimpleParticle {
	DodgeParticle( ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, SpriteSet spriteSet ) {
		...
		this.xdOnGroundFormula = this.xdFormula = xd->xd + Random.nextFloat( -0.0002f, 0.0002f );
		this.ydOnGroundFormula = this.ydFormula = yd->yd - 0.00002f;
		this.zdOnGroundFormula = this.zdFormula = zd->zd + Random.nextFloat( -0.0002f, 0.0002f );
