	...
	public static void onUpdate( LivingEvent.LivingUpdateEvent event ) {
		...
		if( enchantmentLevel > 0 && LevelHelper.isEntityOutsideDuringTheDay( entity ) ) {
			if( TimeHelper.hasServerTicksPassed( 4 ) )
				level.sendParticles( ParticleTypes.SMOKE, entity.getX(), entity.getY( 0.5 ), entity.getZ(), 3 + enchantmentLevel, 0.125, 0.5, 0.125,
					0.01
				);

			if( TimeHelper.hasServerTicksPassed( vampirism.effectCooldown.getDuration() ) ) {
