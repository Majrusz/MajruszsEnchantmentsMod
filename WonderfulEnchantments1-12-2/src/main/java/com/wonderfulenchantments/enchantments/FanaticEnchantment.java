    ...
    public static void fishingFanaticEvent( ItemFishedEvent event ) {
            ...
            if( WonderfulEnchantments.RANDOM.nextFloat() < 0.33334f )
                for( ItemStack itemstack : loottable.generateLootForPools( WonderfulEnchantments.RANDOM, lootContext$builder.build() ) ) {
                    EntityItem entityItem = new EntityItem(
                        world,
                        fishingBobber.posX + 0.50D * WonderfulEnchantments.RANDOM.nextDouble(),
                        fishingBobber.posY + 0.25D * WonderfulEnchantments.RANDOM.nextDouble(),
                        fishingBobber.posZ + 0.50D * WonderfulEnchantments.RANDOM.nextDouble(),
                        itemstack
                    );

                    double	deltaX = player.posX-entityItem.posX,
                            deltaY = player.posY-entityItem.posY,
                            deltaZ = player.posZ-entityItem.posZ;
        ...
        if( tryIncreaseFishingFanaticLevel( player ) )
            player.sendStatusMessage( new TextComponentString(
                TextFormatting.BOLD + new TextComponentTranslation( "wonderful_enchantments.fanatic_level_up" ).getUnformattedText()
            ), true );
